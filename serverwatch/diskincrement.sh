...
function disk_increment() {
	tlist=($(ls -lat $svrlogs/serverwatch | grep -i "diskusage" | grep -i "$(date +"%F")" | head -1 | awk '{print "$svrlogs/serverwatch/"$9}'))
	ylist=($(ls -lat $svrlogs/serverwatch | grep -i "diskusage" | grep -i "$(date -d 'yesterday' +"%F")" | head -1 | awk '{print "$svrlogs/serverwatch/"$9}'))
