...
function account_spf() {
	newdomains=($(ls -lat $temp | grep "createaccount" | grep "$(date +"%F_%H:")" | head -1 | awk '{print $9}'))

	filepath="$temp/$newdomains"
...
function addon_spf() {
	newdomains=($(ls -lat $svrlogs/errorlogwatch | grep "addonlog" | grep "$(date +"%F_%H:")" | head -1 | awk '{print $9}'))

	filepath="$svrlogs/errorlogwatch/$newdomains"
...
function print_data() {
	filename=$(echo "$newdomains" | awk -F'_' '{print $1}')
