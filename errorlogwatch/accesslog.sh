...
function access_log() {
	cat /usr/local/cpanel/logs/access_log | awk -v Date1=$(date -d '12 hours ago' -u +"[%m/%d/%Y:%H:00:00") -v Date2=$(date -d '1 hours ago' -u +"[%m/%d/%Y:%H:59:59") '{if ($4 >= Date1 && $4 <= Date2) print $0}' | awk '{print $1,$3,$4,$6,$9,$NF}' | awk -F'[/ :]' '{if ($2!="-") printf "%-19s %-13s %-16s %-13s %-22s %-13s\n","DATE: "$5"-"$3"-"$4,"TYPE: "$9,"RESPONSE: "$10,"PORT: "$NF,"IP: "$1,"USER: "$2}' | sed 's/[][]//;s/"//g' | sort | uniq -c | sort -nr | grep -ie "POST\|GET" | grep -ie "2086\|2087" >>$temp/whmaccess_$time.txt
}
